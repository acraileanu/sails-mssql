...
utils.convertDates = function (options) {
  ...
  _.each(where, function (attrWhere, attrName) {
    ...
    if (
      ...
      attributes[attrName] &&
      attributes[attrName].autoMigrations &&
      attributes[attrName].autoMigrations.columnType.indexOf("date") === 0
    ) {
      // key pair strings
      if (_.isString(attrWhere)) {
        return (where[attrName] = utils.toSqlDate(
          attrWhere === null ? null : moment(attrWhere)
        ));
      }
      // in pairs
      if (_.isArray(attrWhere)) {
        return (where[attrName] = _.map(attrWhere, function (individualWhere) {
          return utils.toSqlDate(
            individualWhere === null ? null : moment(individualWhere)
          );
        }));
      }
      ...
      if (_.isObject(attrWhere)) {
        _.each(attrWhere, function (modifiedWhere, modifier) {
          ...
          if (_.isArray(modifiedWhere)) {
            return (attrWhere[modifier] = _.map(
              modifiedWhere,
              function (individualWhere) {
                return utils.toSqlDate(individualWhere);
              }
            ));
          }
        });
